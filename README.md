...
The users' graph time series' data used in the article https://doi.org/10.1016/j.frl.2020.101489 can be obtained using this repository or downloaded directly in either of the followinf three formats: [.xlsx](./article_series/time_series.xlsx), [.txt](./article_series/time_series.txt), [.csv](./article_series/time_series.csv). It is also possible to obtain the most recent data to keep the research up-to-date. The scripts contained in this repository can be used to obtain the time series derived from the Bitcoin users' graph. For the information about the time series:
