...
/*void Graph::saveClusteringCoefficientsOverTime()
{
	...
	edgesUpToCurrentTime.clear();
	vector<int> verticesIntervalSubgraph;
	vector<Edge> edgesIntervalSubgraph;
	verticesIntervalSubgraph.clear();
	edgesIntervalSubgraph.clear();
	Graph intervalSubgraph,subgraphUpToCurrentTime;
	for (int i = 0;i < 120;i++)
	{
		intervalSubgraph = loadUsersSubgraph(i,false);
		verticesIntervalSubgraph = intervalSubgraph.getVertices();
		edgesIntervalSubgraph = intervalSubgraph.getEdges();
		vertices2Add.clear();
		FOREACH(v, verticesIntervalSubgraph)
		{
			if (verticesNoRepetitionsUpToCurrentTime.find(*v) == verticesNoRepetitionsUpToCurrentTime.end())
			{
				verticesNoRepetitionsUpToCurrentTime.insert(*v);
				vertices2Add.insert(*v);
			}
		}
		FOREACH(edge, edgesIntervalSubgraph)edgesUpToCurrentTime.push_back(*edge);
		FOREACH(v, vertices2Add)verticesUpToCurrentTime.push_back(*v);
		subgraphUpToCurrentTime = Graph(verticesUpToCurrentTime, edgesUpToCurrentTime);
...
/*void Graph::saveAverageClusteringCoefficientsOverTime()
{
	...
	FILE *save;
	save = fopen("C:\\Users\\Administrator\\Desktop\\bitcoin\\pliki_wegrow_2018_luty\\users_graphs\\avg_clust_coeffs_over_time.txt", "w");
	for (int i = 0;i < 120;i++)
	{
		loadClusteringCoefficients(i);
		clusteringCoefficientSum = 0;
		FOREACH(elt, clusteringCoefficients)clusteringCoefficientSum += elt->second;
...
/*void Graph::loadClusteringCoefficients(int timeId)
{
	...
	while (!feof(read))
	{
		fscanf(read, "%d %lf", &vertex, &coeffx);
